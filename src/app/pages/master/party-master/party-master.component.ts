...
export class partyMasterDialogComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (this.action === 'Edit') {
      ...
      this.partyForm.controls['isBroker'].setValue(this.local_data.isBroker.id)
      this.partyForm.controls['isFirm'].setValue(this.local_data.isFirm.id)
      this.partyForm.controls['broerpercentage'].setValue(this.local_data.broerpercentage)
    }
  }
  ...
  doAction(): void {
    const payload = {
      ...
      isBroker : this.partyForm.value.isBroker,
      broerpercentage : this.partyForm.value.broerpercentage,
    }
  ...
  getFirmList() {
    ...
    this.firebaseService.getAllBroker().subscribe((res: any) => {
      if (res) {
        ...
        if (this.action === 'Edit') {
          const selectedbroker = this.brokerList.find((firm: any) => firm.id === this.local_data.isBroker);
          if (selectedbroker) {
            this.partyForm.controls['isBroker'].setValue(selectedbroker);
          }
        }
