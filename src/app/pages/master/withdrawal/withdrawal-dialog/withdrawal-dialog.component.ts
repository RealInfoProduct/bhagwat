...
export class WithdrawalDialogComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (this.action === 'Edit') {
     this.withdrawalForm.controls['date'].setValue(  new Date(this.local_data.date.seconds * 1000));
      this.withdrawalForm.controls['employee'].setValue(this.local_data.employee)
  ...
  doAction(): void {
    const payload = {
      ...
      employee: this.withdrawalForm.value.employee,
